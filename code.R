      ...
      band.country = 
        c(  c(band.country, " || "),
            band.pages[j] %>% 
              ...
              as.character()%>% 
              str_remove_all(malink) %>% 
              str_remove_all('(<a href=\\"/lists/\\w*\\">)|(</a>)')
        )
    }
...
export.webpage = 
  function(dataname) { 
    ...
    dataname %>%
      ...
      mutate(
        Cover = paste0( album.link %>% str_match('(?<=\\/)\\d[^.]*$'), ## generate id to sort the albums
                        '|||',
                        album.cover
                        ),
        Album = paste0('<a href="',album.link,'">',Title,'</a>'),
        Band = paste0(band.links, 
                      "|||",
                      band.country
                      ),
        Asso = band.related,
